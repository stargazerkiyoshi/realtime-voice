...
## 决策
- 决策：拆分为 Input/VAD、ASR、LLM/TTS 三条管线，协调器仅维护 LISTENING/SPEAKING。
- 决策：将 turn idle 与 ws idle 分离，避免 ASR 连接因静音而频繁重连。
- 决策：暂时关闭 TurnManager，ASR 结果不再由后端主动切分轮次。
- 备选方案：保持现有 eventLoop 结构，仅修补边界条件。原因是难以持续维护，复杂度随功能增加快速上升。
